...
# Este script dispara el webhook de deploy despues de hacer push

Write-Host "Iniciando deploy automatico a Render..." -ForegroundColor Cyan
...
if (Test-Path $envPath) {
    ...
    if ($envContent -match 'RENDER_DEPLOY_HOOK_URL=(.+)') {
        ...
        Write-Host "No se encontro RENDER_DEPLOY_HOOK_URL en .env" -ForegroundColor Red
        exit 1
    }
} else {
